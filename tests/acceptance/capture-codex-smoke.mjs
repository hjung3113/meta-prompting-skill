...
const command = process.argv[2]; const capture = process.argv[3];
...
if (command === "init") {
  ...
  mkdirSync(capture, { recursive: true }); const fresh = join(capture, "fresh-project");
  ...
  atomic(statePath, { fresh, nextIndex: 0, threadId: null, entries: [], skill: realpathSync(resolve(root, "skills/meta-prompt/SKILL.md")), skillSha256: hash(resolve(root, "skills/meta-prompt/SKILL.md")) });
}
const state = JSON.parse(readFileSync(statePath, "utf8"));
if (command === "step") {
  const index = state.nextIndex; if (index >= turns.length) { console.log("complete"); process.exit(0); }
  const args = index === 0 ? ["exec", "--json", "-C", state.fresh, turns[0]] : ["exec", "resume", "--json", state.threadId, turns[index]];
  const result = spawnSync("codex", args, { cwd: state.fresh, input: "", encoding: "utf8", stdio: ["ignore", "pipe", "pipe"], timeout: 180000 });
  const raw = join(capture, `turn-${String(index).padStart(2, "0")}.jsonl`); writeFileSync(raw, result.stdout); writeFileSync(`${raw}.stderr`, result.stderr);
